...
while True:
    ret,frame=video.read() #.read - give to value one boolean and second frames
    Pressingkey.keyPressed = False
    Pressingkey.spacePressed=False
    key_count=0
    key_pressed=0   
    hands,img=detector.findHands(frame)
    cv2.rectangle(img, (0, 480), (300, 425),(50, 50, 255), -2) # making two rectangles two place our text there
    ...
    if hands:
        ...
        if fingerUp==[0,0,0,0,0]:
            cv2.putText(frame, 'Finger Count: 0', (20,460), cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255), 1, cv2.LINE_AA)#(255,..)-- white colour
            cv2.putText(frame, 'Jumping', (440,460), cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255), 1, cv2.LINE_AA)
            Pressingkey.PressKey(space_key_pressed)
            Pressingkey.spacePressed=True
            current_key_pressed.add(space_key_pressed)
            key_pressed=space_key_pressed
            Pressingkey.keyPressed = True
        ...
        if fingerUp==[1,1,1,1,1]:
            ...
            cv2.putText(frame, 'Not Jumping', (420,460), cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255), 1, cv2.LINE_AA)
        if not Pressingkey.keyPressed and len(current_key_pressed) != 0:
            for key in current_key_pressed:
                Pressingkey.ReleaseKey(key)
            current_key_pressed = set()
